// ExpandablePX:  Array of Pointers Extended(only)
...
The DatumPtrX template defines a pointer to the Datum.  A RcdPtr is the item that is stored in the array.
This leads to some efficiency since when moving items in the array, only a pointer is moved.  However, it
also introduces some complexity.  For the most part, ExpandablePX manages allocating and deallocating the
storage used for the Datum but there is room for the user to allocate and deallocate when this is
required.

This extended version of ExpandableP includes a key class in the definitions of the DatumPtrX object and
the ExpandablePX object.  This then requires a class with a minimal set of boolean operations to perform
searches, insertion sorts and binary searches.  The boolean functions needed are described below.
...
The operations supported by an ExpandablePX array where the declaration is:

  typedef DatumPtrX<Datum, KeyClass> DatumP;                // Usually defined just before iterator

  class Xyz {
  ExpandablePX<Datum, KeyClass, DatumP, 2> data;
        ,,,
...
Datum* p;
  ...
  DatumPtrX() : p(0) { }
 ~DatumPtrX() {p = 0;}
  DatumPtrX(DatumPtrX& x) {p = x.p;}

  DatumPtrX& operator=  (Datum& r)  {p = &r;  return *this;}
  DatumPtrX& operator=  (Datum* r)  {p =  r;  return *this;}
  DatumPtrX& operator=  (DatumPtrX& x) {p = x.p; return *this;}

  // Required for Insertion Sort, i.e. data = dtm;
  bool     operator== (DatumPtrX& x) {return *p == *x.p;}
  bool     operator>= (DatumPtrX& x) {return *p >= *x.p;}
  ...
  // Required for Binary Search: where Key& key, *this < key, *this > key, *this == key
  // Binary Search of Key& (Reference to the key for Datum)

  bool     operator== (Key& key) {return (*p) == key;}
  bool     operator!= (Key& key) {return (*p) != key;}

  bool     operator<  (Key& key) {return ((Datum&)(*p)) <  key;}
  bool     operator<= (Key& key) {return (*p) <= key;}
...
template <class Datum, class Key, class RcdPtr, const int n>
class ExpandablePX {
...
public:
  ...
  ExpandablePX();                      // Constructor & Destructor
 ~ExpandablePX();

 ExpandablePX& operator= (ExpandablePX& e);                               // copy the whole array
...
private:
  ...
  void   copy(ExpandablePX& e);                  // Copy array e to this array
...
template <class Datum, class Key, class RcdPtr, const int n>
ExpandablePX<Datum, Key, RcdPtr, n>::ExpandablePX() : endN(0), tblN(n > 0 ? n : 1)
...
template <class Datum, class Key, class RcdPtr, const int n>
ExpandablePX<Datum, Key, RcdPtr, n>::~ExpandablePX()
...
template <class Datum, class Key, class RcdPtr, const int n>
ExpandablePX<Datum, Key, RcdPtr, n>& ExpandablePX<Datum, Key, RcdPtr, n>::operator= (ExpandablePX& e)
...
template <class Datum, class Key, class RcdPtr, const int n>
RcdPtr& ExpandablePX<Datum, Key, RcdPtr, n>::operator[] (int i) {
  if (i >= tblN) expand(i);
  if (i >= endN) endN = i+1;
  return tbl[i];
  }
...
template <class Datum, class Key, class RcdPtr, const int n>
Datum* ExpandablePX<Datum, Key, RcdPtr, n>::operator+= (Datum* r)
...
template <class Datum, class Key, class RcdPtr, const int n>
Datum& ExpandablePX<Datum, Key, RcdPtr, n>::nextData()
...
template <class Datum, class Key, class RcdPtr, const int n>
Datum& ExpandablePX<Datum, Key, RcdPtr, n>::getData(int i) {
...
template <class Datum, class Key, class RcdPtr, const int n>
bool ExpandablePX<Datum, Key, RcdPtr, n>::operator() (int x, Datum& d)
...
template <class Datum, class Key, class RcdPtr, const int n>
bool ExpandablePX<Datum, Key, RcdPtr, n>::del(Datum* p) {
int i;
...
template <class Datum, class Key, class RcdPtr, const int n>
bool ExpandablePX<Datum, Key, RcdPtr, n>::del(int x) {
int i;
...
template <class Datum, class Key, class RcdPtr, const int n>
Datum* ExpandablePX<Datum, Key, RcdPtr, n>::bSearch(Key& key) {
int     beg  = 0;
int     end  = endN;
int     last = -1;
int     i;
...
template <class Datum, class Key, class RcdPtr, const int n>
Datum* ExpandablePX<Datum, Key, RcdPtr, n>::find(Key& key) {
int i;
...
template <class Datum, class Key, class RcdPtr, const int n>
Datum* ExpandablePX<Datum, Key, RcdPtr, n>::insertSorted(Datum* p) {
RcdPtr xNode;
RcdPtr nextNode;
int    i;
...
template <class Datum, class Key, class RcdPtr, const int n>
void ExpandablePX<Datum, Key, RcdPtr, n>::expand(int i) {
RcdPtr* p      = tbl;
RcdPtr* q      = tbl;
int     nItems = tblN;
int     j;
...
template <class Datum, class Key, class RcdPtr, const int n>
void ExpandablePX<Datum, Key, RcdPtr, n>::copy(ExpandablePX& e) {
...
template <class Datum, class Key, class RcdPtr, const int n>
void ExpandablePX<Datum, Key, RcdPtr, n>::freeAllNodes() {
