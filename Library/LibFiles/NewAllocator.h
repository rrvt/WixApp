...
#define AllocArray(n)   MyAllocator(n, _NORMAL_BLOCK, __FILE__, __LINE__)
#else
#define NewAlloc(T)     NewAllocatorT<T> MyAllocator
#define AllocNode       MyAllocator()
#define AllocArray(n)   MyAllocator(n)
#endif

#define FreeNode(p)     MyAllocator.freeNode(p)
#define FreeArray(p)    MyAllocator.freeArray(p)
...
public:
  ...
  T* operator() (int blk, const char* file, int line)
    {T* p = (T*) _malloc_dbg(sizeof(T), blk, file, line);   if (p) {new(p) T();}   return p;}
