    ...
    if (m_pszMsg == NULL) {
        ...
        if (m_pszMsg != NULL) {
            ...
            if (m_pszMsg != NULL) {
                WORD wCode = WCode();
                if (wCode != 0) {
                    _COM_PRINTF_S_1(m_pszMsg, 32, _T("IDispatch error #%d"), (int)wCode);
                }
                else {
                    _COM_PRINTF_S_1(m_pszMsg, 32, _T("Unknown error 0x%0lX"), m_hresult);
                }
            }
        }
    }
...
{
    return wCode >= 0xFE00 ? WCODE_HRESULT_LAST : (long) WCODE_HRESULT_FIRST + wCode;
}
