...
Datum is a class, e.g. class Datum {...}, and requires the following methods:
  ...
  -- Copy Constructor, e.g. Datum(const Datum& d), which copies all components of object d to
     object *this
  -- Assignment operator, e.g. Datum& operator= (Datum& d), which copies all components of object
     d to object *this and returns *this
...
allocated content) then the Strings must be allowed to destruct normally and the pointers must just
be zeroed.

The DatumPtrT template defines a pointer to the Datum.  A DatumPtr is the item that is stored in
the array.  This leads to some efficiency since when moving items in the array, only a pointer is
moved.  However, it also introduces some complexity.  For the most part, ExpandableP manages
allocating and deallocating the storage used for the Datum but there is room for the user to
allocate and deallocate when this is required.  This is the declaration needed to define the
pointer object:

  -- typedef DatumPtrT<Datum, Key> DatumP;

This extended version of ExpandableP includes a key class in the definitions of the DatumPtrT
object and the ExpandableP object.  This then requires a class with a minimal set of boolean
operations to perform searches, sorts and binary searches.
...
file contains a template for creating an iterator for Expandable files.  First one must declare the
Iterator class so that an object may be created somewhere that it is going to be used.  The Datum
class is a record that is stored in an expandable array which is housed in class Xyz
...
Now that is not all there is to do.  The Xyz class must implement the following:
  ...
  Data* datum(int i)       -- returns either a pointer to data (or datum) at index i in array or
                              zero
...
the operations supported are:
  ...
  Datum& d = data.getData(i);   // return a reference to a record at index i allocating a record if
                                // necessary
  data(i, datum);               // datum is inserted at index i, the contents at i and above are
                                // moved up one element.  A new record is allocated and datum
  ...
  data.del(i);                  // The datum at index i is deleted and the elements above are moved
                                // down to fill in the hole.  The number of elements in the array
                                // is reduced by one
  data.find(key)                // Performs a linear search for key.  Requires datum == Key object,
                                // where Key is a type found in Datum object
  data.bsearch(key)             // Performs a binary search for key.  Requires a sorted array and
                                // the boolean operations datum > key, datum < key, datum == key
  Datum* d = data.allocate();   // Allocate one record and return a pointer to it -- use in
                                // moderation
...
DatumPtr* tbl;                          // Pointer to a heap object which is treated as an array of
                                        // Datums
public:
