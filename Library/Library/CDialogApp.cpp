...
CDialogApp::CDialogApp(CDialogApp* app) /*: doc(0), view(0)*/ {
ResourceData res;
...
#if 0
void CDialogApp::setAppName(TCchar* appName)
                                         {if (getMainFrame()) getMainFrame()->setAppName(appName);}


void CDialogApp::setTitle(  TCchar* rightPart)
                                         {if (getMainFrame()) getMainFrame()->setTitle(rightPart);}


CDocument* CDialogApp::getDoc() {
...
#if 0
HANDLE CDialogApp::swapDevMode(HANDLE newDevMode)
                                      {HANDLE h = m_hDevMode; m_hDevMode = newDevMode;  return h;}
#endif
#if 0
void CDialogApp::initPrinterAttr() {
...
void CDialogApp::savePrinterAttr() {
HANDLE   hdl = getDevMode();
DEVMODE* devMode;
...
String CDialogApp::getPrinterName() {
HANDLE   hdl = getDevMode();
DEVMODE* devMode;
String   name;
