...
static TCchar* Explanation =
                      _T("WixApp will produce a wix file \"Product.wxs\" when the Create Button ")
...
String s = rootPath + _T('\\');
  ...
  if (solution.getSolutionPath(s))
                                  {solutionPath = s;  solutionPathCtl.SetWindowText(solutionPath);}
  }
