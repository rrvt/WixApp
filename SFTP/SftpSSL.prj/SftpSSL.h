...
//  bool readPending();
  ...
  bool date(TCchar* path, Date& val)  {return cmd.date(path, val);}     // File last modified date when
                                                                        // true

  bool stor(TCchar* webPath) {return cmd.stor(webPath);}  // copy sftpTransport buffer to web host file
  bool stou(TCchar* webPath, String& fileName)            // copy transport buffer to unique file name
                    {return cmd.stou(webPath, fileName);} // in current direcotry

  bool append(TCchar* webPath) {return cmd.append(webPath);}  // Append sftpTransport buffer to web host
                                                              // file

  bool retr(TCchar* webPath) {return cmd.retr(webPath);}  // copy file from web host to sftpTransport
                                                          // buffer
  ...
  bool sendCmd(TCchar* cmmd, TCchar* args, String& response) {return cmd.sendCmd(cmmd, args, response);}
